/*
 * Generates public/explainer-loc.mp4 — a ~60s vertical narrated explainer
...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>
  * { margin:0; box-sizing:border-box; font-family:-apple-system,'Segoe UI',Roboto,sans-serif; }
  ...
  .redbox { background:rgba(239,68,68,.12); border:2px solid rgba(239,68,68,.45); border-radius:22px;
            padding:28px 34px; margin-top:10px; }
  .red { color:#f87171; font-size:52px; font-weight:900; }
  .vsrow { display:flex; gap:14px; margin-top:18px; width:100%; }
  .vsbox { flex:1; border-radius:16px; padding:16px 10px; text-align:center; }
</style></head><body>

<!-- Scene 0: intro question -->
<div class="scene" id="s0" style="background:linear-gradient(160deg,#1e40af,#0f172a);color:#fff">
  <div class="em">🤔</div>
  <div class="big" style="font-size:34px">Need $50<br>before payday?</div>
  <div class="sub">Here's what it <em>actually</em> costs<br>at your credit union.</div>
</div>

<!-- Scene 1: 13% APR, by law -->
<div class="scene" id="s1" style="background:#0f172a;color:#fff">
  ...
  <div class="greenbox">
    ...
    <div style="color:#86efac;font-size:22px;font-weight:700">APR — set by law</div>
  </div>
  <div class="sub" style="font-size:14px">Federal credit unions are capped at 18%.<br>Genesee charges 13%.<br>Not a promo rate. Not a trick.</div>
</div>

<!-- Scene 2: one advance, one weekend payback -->
<div class="scene" id="s2" style="background:#f8fafc;color:#0f172a">
  <div class="big" style="font-size:28px;margin-bottom:18px">Borrow Monday,<br>repay Saturday.</div>
  <div class="pill" id="p0">💵 Advance: $${ADVANCE}</div>
  <div class="pill" id="p1">📅 Held 6 days</div>
  <div class="pill" id="p2">💰 Interest: ${cents(interest1)}</div>
</div>

<!-- Scene 3: repeat every payday -->
<div class="scene" id="s3" style="background:#1e293b;color:#fff">
  <div class="sub" style="margin-bottom:10px">Do this every other week</div>
  <div style="font-size:44px;font-weight:900;color:#4ade80">${advances}×</div>
  <div class="sub">$50 advances a year<br>paid back each payday weekend</div>
  <div class="sub" style="margin-top:16px;font-size:15px;opacity:.7">Same rhythm as a cash advance app —<br>but through your credit union.</div>
</div>

<!-- Scene 4: annual cost green box -->
<div class="scene" id="s4" style="background:#0f172a;color:#fff">
  <div class="sub" style="margin-bottom:8px">Your total cost for the year</div>
  <div class="greenbox">
    <div class="green">${cents(annualInterest)}</div>
    <div style="color:#86efac;font-size:20px;font-weight:700">in interest — all year</div>
  </div>
  <div class="sub">No fees. No tips. No membership.<br>No surprises.</div>
</div>

<!-- Scene 5: vs. app comparison -->
<div class="scene" id="s5" style="background:#0f172a;color:#fff">
  <div class="sub" style="margin-bottom:10px">Same $50. Same cycle. Different story.</div>
  <div class="vsrow">
    <div class="vsbox" style="background:rgba(239,68,68,.12);border:2px solid rgba(239,68,68,.4)">
      <div style="font-size:13px;font-weight:700;color:#fca5a5;margin-bottom:6px">Cash advance app</div>
      <div style="font-size:32px;font-weight:900;color:#f87171">$${appLow}–<br>$${appHigh}</div>
      <div style="font-size:11px;color:#fca5a5;margin-top:4px">fees, tips &amp; membership</div>
    </div>
    <div class="vsbox" style="background:rgba(34,197,94,.12);border:2px solid rgba(34,197,94,.4)">
      <div style="font-size:13px;font-weight:700;color:#86efac;margin-bottom:6px">Credit union LOC</div>
      <div style="font-size:32px;font-weight:900;color:#4ade80">${cents(annualInterest)}</div>
      <div style="font-size:11px;color:#86efac;margin-top:4px">interest only — nothing else</div>
    </div>
  </div>
</div>
...
<div class="scene" id="s6" style="background:linear-gradient(160deg,#16a34a,#14532d);color:#fff">
  <img src="file://${ICON}" width="130" height="130" style="margin-bottom:22px">
  <div class="big" style="font-size:30px">It's your money.<br>Keep it.</div>
